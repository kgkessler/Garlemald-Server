...
//! Map server as a library.
//!
//! The map server is built as a `lib` + thin `bin` so its zone-simulation
//! and Lua content runtime can be driven in-process by tooling other than
//! the live network server, notably the `content-test` Lua quest-walkthrough
//! runner, which embeds [`lua::LuaEngine`] directly.
//!
//! `src/main.rs` is a one-line shim over [`run`]. The `lua` module is the
//! only intentionally-public surface today; everything else stays crate-private
//! and is reachable via `crate::` from within the lib (including the
//! `#[cfg(test)]` integration suite under `runtime`).
...
mod inventory;
mod leve;
// The Lua content runtime is the one surface the in-process test harnesses
// (e.g. the `content-test` quest-walkthrough exe) link against.
pub mod lua;
mod npc;
mod packets;
mod processor;
...
mod social;
mod status;
// Opt-in headless test-harness facade for the `content-test` runner, gated
// behind the `testkit` cargo feature so it never ships in the server build.
#[cfg(feature = "testkit")]
pub mod testkit;
mod world_manager;
mod zone;
...
/// Boot and run the live map server: load config, open the DB, populate the
/// Lua catalogs, spawn the game-loop ticker + GM console reader, and serve.
///
/// This is the body that used to live in `main()`; `src/main.rs` is now a
/// thin `#[tokio::main]` shim that just `await`s this.
